/// Thrown by ``PBKDF2/deriveKey(password:salt:iterations:keyLength:deadline:)`` when its
/// `deadline` passes. Kept internal, and deliberately *not* `PerunError.timedOut` (which promises a
/// still-usable connection): when this fires during connect the socket is torn down, so the connect
/// path maps it to a connection failure instead.
struct KeyDerivationDeadlineExceeded: Error {}
...
enum PBKDF2 {
    ...
    /// ``KeyDerivationDeadlineExceeded`` once it passes. The iteration count is server-controlled
    /// and, in SCRAM, consumed before the server is authenticated — so an inflated count is a
    /// CPU-exhaustion DoS. The deadline keeps that CPU work bounded by the caller's budget (the
    /// connect deadline) rather than running to completion after the socket has already been torn down.
            ...
            if iterations > 1 {
                for iteration in 2 ... iterations {
                    if let deadline, iteration & 0x3FF == 0, ContinuousClock().now >= deadline {
                        throw KeyDerivationDeadlineExceeded()
                    }
